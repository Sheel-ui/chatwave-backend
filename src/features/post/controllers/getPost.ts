...
export class Get {
    public async posts(req: Request, res: Response): Promise<void> {
        ...
        // Calculate skip and limit values based on the page and PAGE_SIZE
        ...
        // Check if there are cached posts
        if (cachedPosts.length) {
            posts = cachedPosts;
            totalPosts = await postCache.getTotalPostsInCache();
        } else {
            // Retrieve posts from the service if no cached posts are available
    ...
    public async postsWithImages(req: Request, res: Response): Promise<void> {
        ...
        // Fetch posts with images from the service if not available in the cache
